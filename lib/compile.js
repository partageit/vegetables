...
module.exports = function(config, file, filesToRender, callback) {
	...
	commonTags.slideshow = merge(true, config.tags.slideshow, wildcardPageOptions.slideshow, pageOptions.slideshow, documentTags.slideshow);

	// Get page layout **********************************************************
	if (commonTags.layout) {
		var versions = templatesManager.getVersions(file.path, 'layout-' + commonTags.layout, file.isIndex);
	...
	Object.keys(file.versions).forEach(function(versionFormat) {
		...
		// Warning: helpers should be available in compile-special-pages.js too...
		// For now it is copied, but a better solution should be implemented
		var helpers = {
			'active': function(uri) {
