...
function applyFilters(stage, config, filters, content) {
	...
	for (var i = 0; i < filters.length ; i++) {
		...
		if (config.makeInput) {
			content = config.makeInput(content, "module" in filter, config)
		}

		if (typeof filter === "string") {
			content = child_process.execSync(filter, {'input': content})
		} else {
			// is object
			if (filter.module) {
                                var modulefile = filter.module
				if (!path.isAbsolute(modulefile)) {
					if (config.configFile) {
						modulefile = path.resolve(path.dirname(config.configFile), modulefile)
					}
					else {
				...
				if (mod instanceof Function) {
					content = mod({"stage": stage, "content": content, "config": config})
				}

			} else {
				commandOptions = merge(true, filter.options, {'input': content})
				content = child_process.execSync(filter.command, commandOptions)
			}
		}

		if (config.makeOutput) {
			content = config.makeOutput(content, "module" in filter)
		}
	}
	return content;
}
...
module.exports = function(config, file, filesToRender, callback) {
	...
	if (config.filterMarkdown) {
		md = applyFilters('markdown', config, config.filterMarkdown, md);
	}
	...
	if (config.filterHTML) {
		content = applyFilters('html', config, config.filterHTML, content);
	}
	...
	Object.keys(file.versions).forEach(function(versionFormat) {
		...
		if (config.setupTags) {
			...
			for (var i = 0; i < setupTags.length; i++) {
				tags = require(setupTags[i])(config, tags);
			}
		}
