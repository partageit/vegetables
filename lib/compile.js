...
module.exports = function(config, file, filesToRender, templatesCache, callback) {
	...
	file.versions.forEach(function(version) {
		...
		if (!templatesCache[version.templateFile]) {
			logger.debug('Template %s was not in cache', version.templateFile);
			try {
				templatesCache[version.templateFile] = fs.readFileSync(version.templateFile, {encoding: 'utf-8'});
			} catch (e) {
				logger.error('Template read error "%s": %s', version.templateFile, e);
				return;
			}
		}
		...
		}, config.tags, pageOptions, documentTags); // Configuration and in-document tags
		try {
			var rendered = mustache.render(templatesCache[version.templateFile], tags);
