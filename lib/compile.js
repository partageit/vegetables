...
module.exports = function(config, file, filesToRender, callback) {
	...
	//dataIndex.addPage(title, title + '.html', content, '');
	...
	file.versions.forEach(function(version) {
		...
		handlebars.registerHelper('active', function(uri) {
			return (uri === version.uri ? 'active' : '');
		});

		handlebars.registerHelper('activeDocument', function(uri) {
			return (uri === file.path.replace(/\\/g, '\/') ? 'active' : '');
		});

		handlebars.registerHelper('uriByVersion', function(format) {
			var uri = '';
