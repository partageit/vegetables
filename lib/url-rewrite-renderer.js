...
module.exports = function (renderer, baseUri, file, filesToRender) {
	...
	renderer.link = function(href, title, text) {
		...
		// Handle gollum-like links, i.e. without extension
		if (!filesToRender[originalDocument]) {
			if (filesToRender[originalDocument + '.md']) {
				originalDocument += '.md';
			} else if (filesToRender[originalDocument + '.markdown']) {
				originalDocument += '.markdown';
			}
		}
