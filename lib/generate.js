...
module.exports = function(argv, init, callback) {
	...
	templatesManager.scanPartials();

	// Clear and configure search engine data
	//dataIndex.init({});
	...
	fs.copyRecursive(path.join(config.template, 'assets'), path.join('.vegetables', 'website', 'assets'), function(err) {
		...
		async.each(
			...
			function(err) {
				...
				// Write search engine data
				//if (config.searchEngine) {
				//	dataIndex.writeIndex(path.join('.vegetables', 'website', 'index.json'));
				//}
				// Compile special pages here
