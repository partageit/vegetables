...
module.exports = function (argv) {
	...
	server.on('error', function(error) {
		logger.error('Error while serving Vegetables');
		if (error.code === 'EADDRINUSE') {
			logger.error('Another Vegetables instance seems to be runnning.');
			logger.info('Change port to start many instances, or stop the first one.');
			logger.info('Type "vegetables --help" for further details.');
		} else {
