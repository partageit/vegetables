...
var templatesManager = function templatesManager() {
	...
	this.getFolderIndex = function(files) {
		var lowerCaseFiles = files.map(function(file) { return file.toLowerCase(); });
		var potentialIndexes = ['readme.md', 'readme.markdown', 'home.md', 'home.markdown'];
		for (var i = 0 ; i < potentialIndexes.length ; i++) {
			if (lowerCaseFiles.indexOf(potentialIndexes[i]) !== -1) {
				return files[lowerCaseFiles.indexOf(potentialIndexes[i])];
			}
		}
		return '';
	}
	...
	this.getVersions = function(filename, layout, indexFile) {
		if (!templates[layout]) {
		...
		templates[layout].forEach(function(template) {
			versions[template.suffix] = {
				...
				uri: path.join(
					...
					(path.basename(filename) === indexFile ? 'index' : path.basename(filename, path.extname(filename))) +
