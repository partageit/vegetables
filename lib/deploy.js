...
module.exports = function (argv) {
	...
	var finish = function(error, stdout, stderr) {
		if (error) {
			logger.error('Web site not deployed:', stdout, stderr);
		} else {
			logger.success('Web site successfully deployed');
		}
		removeTempDir();
	};
	...
	var gitAdd = function(error) {
		if (error) {
			logger.error('Unable to copy the generated Web site to the deploy folder: %s', error);
			removeTempDir();
			return;
		}
		process.exec(
	...
	var mirror = function(error) {
		if (error) {
			logger.error('Unable to generate the Web site: %s', error);
			removeTempDir();
			return;
		}
		logger.success('Web site generated');
	...
	var generate = function(error, stdout, stderr) {
		...
		require('./generate')(argv, false, mirror);
	};
